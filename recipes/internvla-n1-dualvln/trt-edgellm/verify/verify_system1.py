...
def out_of(o, key):
    if isinstance(o, dict):
        return o.get(key, next(iter(o.values())))
...
def main():
    ...
    print("[4/4] Compare\n" + "=" * 56)
    d = (tref - ttrt).norm(dim=-1)
    endp = (tref[:, -1] - ttrt[:, -1]).norm(dim=-1)
    ...
    print(
        f"  Latency S1: PyTorch {pt_ms:.1f}ms ({1000/pt_ms:.1f}Hz) → full-TRT {trt_ms:.1f}ms ({1000/trt_ms:.1f}Hz) = {pt_ms/trt_ms:.2f}x")  # noqa: E501
    return 0
