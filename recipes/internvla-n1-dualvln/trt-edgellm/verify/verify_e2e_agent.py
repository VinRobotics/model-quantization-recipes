...
from engine_runner import build_mrope_table, run_engine, ENGINE   # FP8 LLM engine harness
...
TRAJDIT=os.path.join(os.environ.get("WORK_DIR",os.path.expanduser("~/vln-opt-work")), "onnx/system1_traj_dit_async_bf16.engine")
...
TRAJ_TOKEN_INDEX=151667; IMAGE_TOKEN_INDEX=151655; SEED=12345


def out_of(o,k):
    if isinstance(o,dict): return o.get(k, next(iter(o.values())))
...
def main():
    ...
    _s=importlib.util.spec_from_file_location("iar",os.path.join(ACTIVE,"internnav/agent/internvla_n1_agent_realworld.py"))
    ...
    class A: device="cuda:0"; model_path=CKPT; model_path_original=CKPT; resize_w=384; resize_h=384; num_history=8; plan_step_gap=4
    print("[1/5] Load agent + engines")
    agent=Agent(A())
    agent.save_dir=os.path.join(SCRATCH,"trtagent_dbg"); os.makedirs(agent.save_dir,exist_ok=True)
    model=agent.model; m=model.get_model()
    ...
    def trt_generate_traj(traj_latents, images_dp, depths_dp=None, predict_step_nums=32,
        ...
        with torch.no_grad():
            ...
            xdp=images_dp.permute(0,1,4,2,3); xdp=((xdp-rmean)/rstd).flatten(0,1)
            ...
            bs=tl.shape[0]
            latents=randn_tensor((bs*num_sample_trajs,predict_step_nums,3),generator=None,device=dev,dtype=dtype)
            sch=FlowMatchEulerDiscreteScheduler(); sch.set_timesteps(num_inference_steps,sigmas=np.linspace(1.0,1/num_inference_steps,num_inference_steps))
            ...
            for t in sch.timesteps:
                ...
                lf=lf+m.pos_encoding(pid); lmi=lf.repeat(2,1,1)
                if hasattr(sch,"scale_model_input"): lmi=sch.scale_model_input(lmi,t)
                tt=t.unsqueeze(0).expand(lmi.shape[0]).to(dev,torch.long)
                npd=out_of(dit_eng(x=lmi.float().contiguous(),timestep=tt.to(torch.int64).contiguous(),z_latents=hs_in.float().contiguous()),"output").to(dtype)
                npd=m.action_decoder(npd); unc,cnd=npd.chunk(2); npd=unc+guidance_scale*(cnd-unc)
    ...
    def seeded_base_traj(*a,**k):
        torch.manual_seed(SEED); np.random.seed(SEED); return base_traj(*a,**k)
    ...
    def run(tag):
        agent.reset(); agent.save_dir=os.path.join(SCRATCH,"trtagent_dbg")
        outs=[]
        for p in rgbs:
            ld=('look_down' in p); rgb=np.asarray(Image.open(p).convert('RGB'))
            depth=10*np.ones(rgb.shape[:2],np.float32); pose=np.eye(4)
            try:
                with torch.no_grad(): o=agent.step(rgb,depth,pose,instr,intrinsic=INTR,look_down=ld)
    ...
    n=min(len(ref),len(trt)); mact=0; nact=0; trajerr=[]
    for i in range(n):
        rk,rf,rv=ref[i]; tk,tf,tv=trt[i]
    ...
    print(f"  outputs: pytorch={len(ref)} trt={len(trt)} (type match {sum(1 for i in range(n) if ref[i][0]==trt[i][0])}/{n})")
    if nact: print(f"  action match: {mact}/{nact}")
    if trajerr:
        import statistics
        print(f"  trajectory per-wp L2 (m): mean={statistics.mean(trajerr):.4f} max={max(trajerr):.4f} (n={len(trajerr)})")
