...
class Args:
    device="cuda:0"; model_path=CKPT; model_path_original=CKPT
    resize_w=384; resize_h=384; num_history=8; plan_step_gap=4
...
def main():
    ...
    agent = Agent(Args())
    dbg = os.path.join(SCRATCH, "verb_dbg"); os.makedirs(dbg, exist_ok=True)
    IMG_DIR = os.path.join(SCRATCH, "verb_imgs"); os.makedirs(IMG_DIR, exist_ok=True)
    ...
    for scene in scenes:
        ...
        agent.reset(); agent.save_dir = dbg
        for p in rgb_paths:
            ...
            try:
                ...
                print(f"    {os.path.basename(p)}: step err {type(e).__name__}: {e}"); continue
            ...
            for turn in agent.conversation_history:
                ...
                for it in turn["content"]:
                    if it["type"] == "image":
                        ...
                        it["image"].save(fp); content.append({"type":"image","image":fp}); ii += 1
                    else:
            ...
            samples.append(dict(scene=sname, img=os.path.basename(p), look_down=look_down,
                                ref=ref_out, S=int(cap["input_ids"].shape[1]) if cap.get("input_ids") is not None else -1,
                                messages=msgs))
            print(f"    {os.path.basename(p):32} look_down={look_down} S={samples[-1]['S']} ref={ref_out!r}")

    in_json = os.path.join(SCRATCH, "verb_in.json"); out_json = os.path.join(SCRATCH, "verb_out.json")
    ...
    import re
    def xy(t):
        d=[int(c) for c in re.findall(r"\d+",t)]; return (d[0],d[1]) if len(d)>=2 else None
    ex=br=0; l2=[]
    for i,s in enumerate(samples):
        ...
        ex+=s["ref"]==fp; br+=rb==fb
        a,b=xy(s["ref"]),xy(fp)
        if a and b: l2.append(((a[0]-b[0])**2+(a[1]-b[1])**2)**.5)
    ...
    if l2: print(f"  coord-L2 px  : mean={sum(l2)/len(l2):.1f} median={sorted(l2)[len(l2)//2]:.1f} max={max(l2):.0f} (n={len(l2)})")
    tag = os.path.basename(os.path.dirname(ENGINE)).replace("edgellm_engines_","")
    json.dump(samples, open(os.path.join(os.environ.get("VLN_OPT_OUT",os.path.expanduser("~/vln-opt-work/out")),f"verbatim_{tag}.json"),"w"),
