...
from traj_dit_loader import load_traj_dit  # noqa: E402
...
ENG = os.path.join(
    os.environ.get(
        ...
        os.path.expanduser("~/vln-opt-work")),
         "onnx/system1_traj_dit_bf16.engine")  # noqa: E131
...
def main():
    ...
    class W(torch.nn.Module):
        ...
        def forward(s, x, timestep, z_latents): return s.d(x=x, timestep=timestep, z_latents=z_latents)  # noqa: E704
    w = W(dit).eval()
    with torch.no_grad():
        ref = w(x, ts, z)
    ...
    wanted = [ln for ln in r.stdout.splitlines()
              if any(w in ln for w in ("successfully", "PASSED", "FAILED"))]
    print("  " + "\n  ".join(wanted)[-400:])
