...
def main():
    ...
    images_dp = torch.stack([tt, tt]).unsqueeze(0).to(dev)  # [1,2,224,224,3]
    ...
    print(
        f"      build {'OK' if os.path.exists(ENG) else 'FAIL'} | {os.path.getsize(ENG)/1e6 if os.path.exists(ENG) else 0:.1f} MB")  # noqa: E501
